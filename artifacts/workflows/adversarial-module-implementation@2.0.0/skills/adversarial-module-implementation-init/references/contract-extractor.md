...
> 本文档为 `adversarial-module-implementation-init` Skill 的契约提取和模糊边界显式仲裁提供详细的程序化解析算法。
...
**冲突仲裁规则**：P0 > P1 > P2。同优先级的冲突记录到 `conflict_log`，由仲裁流程确定。

---

## 模糊边界显式仲裁规则

> **核心要求**：设计文档中所有模糊或不确定的边界描述必须在契约提取阶段显式确定，不得以「约束未声明」搁置。
...
当通过 contract-update 模式增量更新时，在文件头部追加：
