...
| `{module_code_dir}` | 模块代码目录路径 | 初始化产物 | 1, 3 |
| `{spec_path}` | 落地规范文件路径 | 初始化定位 | 1, 2, 3 |
| `{design_doc_path}` | 设计文档路径 | 初始化定位 | 1 |
| `{structure_doc_path}` | 项目结构文档路径 | 初始化定位 | 1 |
| `{contract_expectations_path}` | 契约期望清单路径 | 初始化产物 | 1, 2, 4 |
| `{function_signatures_path}` | 函数签名清单路径 | 验证产物 | 2, 4 |
| `{tech_stack}` | 技术栈描述 | 项目配置 | 2 |
| `{output_dir}` | 测试代码输出目录 | 编排器指定 | 2 |
| `{failure_summary_path}` | 失败摘要路径 | 盲测产物 | 3 |
| `{implementation_dir}` | 实现代码目录 | 编排器指定 | 3 |
| `{test_file_path}` | 测试代码文件路径 | 编排器指定 | 4 |
| `{defects_report_path}` | 测试缺陷报告路径 | 盲测产物 | 4 |
...
| 规则 | 角色 | 约束 | 执行方式 |
|:---|:---|:---|:---|
| **ISO-001** | 实现执行者 | SubAgent 禁止读取 `.tmp/adversarial-tests/` 目录及任何内容 | Prompt 铁律 + `check_isolation.py` 事后审计 |
| **ISO-002** | 测试生成者 | SubAgent 禁止读取实现源码文件 | Prompt 铁律 + `check_isolation.py` 事后审计 |
| **ISO-003** | 修复执行者 | SubAgent 仅可读 failure-summary（信息隔离版本），禁读测试代码 | `validate_failure_summary.py` 阻断级门控 + Prompt 铁律 |
| **ISO-004** | 测试修复者 | SubAgent 仅可读 test-defects（不含测试代码片段和输入值），禁读实现代码 | Prompt 铁律 + 事后审计 |
