...
- **确认点**：否
- **Retry**：1 次
- **描述**：消费领域配置 + s01 产出的 Markdown，自动分析并产出带层级 `[[双链]]` 的 MOC（Map of Content），附 Mermaid 逻辑脉络图。MOC 是后续所有原子笔记的骨架，Skill 内部执行完整质量自检。

### s03-atomic-produce — 原子笔记批量生产
- **目标**：`atomic-producer`
- **确认点**：否
- **Retry**：2 次
- **描述**：单实例顺序遍历 MOC 中全部 H2 模块，逐个生产符合领域写作规范的原子笔记 .md 文件。失败模块在 MOC 中标注 warning，失败隔离（单个模块失败不影响其他模块）。

### s04-quality-audit — 格式审校与交付确认
- **目标**：`quality-auditor`
- **确认点**：是
- **Retry**：1 次
- **描述**：轻量格式审计——检查原子笔记格式合规、文件存在性、MOC 双链有效性。输出简化质量报告。用户基于报告做出决策：
  - **confirmed**：接受交付，进入 s99 终止
  - **rejected**：打回重做，回退到 s03 重新生产（最多 5 轮）
  - **超限**：5 轮后仍未通过，自动终止
...
```
results/workflows/study-note-processor@1.3.0/
├── WORKFLOW.yaml
├── WORKFLOW.md
├── references/
