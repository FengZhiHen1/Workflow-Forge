---
name: "对抗性模块实现流水线"
description: "存量检测→增量路径判定→定向实现与对抗验证→报告 v2.0.1。4 种增量场景（full/incremental/code_only/conflict），共享盲测收敛区。已按 Workflow v3.0.0 规范修正字段与条件值。"
tags: [adversarial, implementation, testing, incrementality, blindtest, reverse-engineering, diff-arbitration]
---

# adversarial-module-implementation@2.0.1

> 存量检测 -> 增量路径判定 -> 定向实现/测试 -> 盲测收敛 -> 报告
>
> **规范化说明**：本版本已按 Workflow v3.0.0 规范修正——移除 `confirmation_point` 字段、`condition: confirmed/rejected` 统一为 `success` + `choice`、`loop_counter_stage` 字段移除。业务逻辑与路由关系不变。
...
- **Workflow v3.0.0 规范化**：移除 `confirmation_point` 字段（由 Skill 内部 AskUserQuestion 替代）；`confirmed`/`rejected` 条件值统一为 `success` + `choice`；移除非规范字段 `loop_counter_stage`
...
flowchart TD
    ...
    s02 -->|"success: full_implementation"| s03f
    s02 -->|"success: incremental_update"| s03id
    s02 -->|"success: code_only"| s03cr
    s02 -->|"success: code_design_conflict"| s03cd
    s02 -->|"success: 放弃"| s99

    %% 路径 1 边
    s03f -->|"success: 确认"| s04f
    s03f -.->|"success: 重做 ≤2"| s03f
    s03f -->|"success: 放弃"| s99
    s03f -.->|loop_exceeded| s99
    s04f -->|success| s05f
    s04f -->|failure| s99
    s05f -->|"success: 接受"| s06f
    s05f -->|"success: 退回 或 failure ≤3"| s04f
    ...
    s04ic -->|"success: 确认"| s05ii
    s04ic -.->|"success: 继续完善 ≤3"| s04ic
    s04ic -->|"success: 放弃"| s99
    s04ic -.->|loop_exceeded| s99
    s05ii -->|success| s06iv
    s05ii -->|failure| s99
    s06iv -->|"success: 接受"| s07it
    s06iv -->|"success: 退回 或 failure ≤3"| s05ii
    s06iv -.->|loop_exceeded ≤3| s13
    s07it -->|success| s10
    s07it -->|failure| s99

    %% 路径 3 边
    s03cr -->|"success: 确认逆推结果"| s04cc
    s03cr -.->|"success: 继续完善 ≤3"| s03cr
    s03cr -->|"success: 放弃"| s99
    s03cr -.->|loop_exceeded| s99
    s04cc -->|"success: 确认"| s05ct
    s04cc -.->|"success: 继续完善 ≤3"| s04cc
    s04cc -->|"success: 放弃"| s99
    ...
    s04ca -->|"success: 确认裁决"| s05cr
    s04ca -.->|"success: 继续仲裁 ≤3"| s04ca
    s04ca -->|"success: 放弃"| s99
    s04ca -.->|loop_exceeded| s99
    s05cr -->|success| s06cv
    s05cr -->|failure| s99
    s06cv -->|"success: 接受"| s07cte
    s06cv -->|"success: 退回 或 failure ≤3"| s05cr
    s06cv -.->|loop_exceeded ≤3| s13
    s07cte -->|success| s10
    s07cte -->|failure| s99

    %% 共享收敛区边
    s10 -->|"success: all_pass | max-rounds | stagnation | user-terminated"| s13
    s10 -->|"success: fix_impl"| s11
    s10 -->|"success: fix_test"| s12
    ...
    s12 -->|failure| s99
    s13 -->|"success: 接受并结束"| s99
    s13 -->|"success: 追加修复"| s10
...
| Stage ID | 名称 | Skill | 模型 | 超时(s) | exclusive | 所属路径 | 门控说明 |
|----------|------|-------|------|---------|-----------|---------|----------|
| s00-workflow-start | 工作流启动 | — (虚拟) | — | — | — | 共用 | — |
| s01-detect | 存量制品检测 | existing-asset-detector (NEW) | light | 120 | — | 共用 | 无门控 |
| s02-path | 增量路径判定 | incremental-path-determiner (NEW) | standard | — | — | 共用 | 自动判定优先，边界时 AskUserQuestion |
| s03-full-init | 环境就绪与契约冻结 | contract-initializer | standard | 300 | — | full | AskUserQuestion：确认/重做/放弃 |
| s04-full-impl | 实现落地执行 | impl-executor (模式A) | heavy | 900 | **yes** | full | 无门控 |
| s05-full-validate | 实现输出验证 | impl-validator | standard | 120 | — | full | 条件触发：重大风险时 AskUserQuestion |
| s06-full-testgen | 对抗性测试生成 | test-generator (模式A) | heavy | 600 | **yes** | full | 无门控 |
| s03-incr-diff | 设计变更差异分析 | diff-arbitrator (NEW, 模式A) | standard | 300 | — | incr | 无门控 |
| s04-incr-contract | 增量契约更新 | contract-initializer (contract-update) | standard | 300 | — | incr | AskUserQuestion：确认/继续完善/放弃 |
| s05-incr-impl | 增量实现更新 | impl-executor (模式C) | heavy | 600 | **yes** | incr | 无门控 |
| s06-incr-validate | 增量实现验证 | impl-validator | standard | 120 | — | incr | 条件触发：重大风险时 AskUserQuestion |
| s07-incr-test | 增量测试更新 | test-generator (模式C) | heavy | 600 | **yes** | incr | 无门控 |
| s03-code-reverse | 逆向推导设计意图 | reverse-engineering-analyzer (NEW) | heavy | 900 | **yes** | code | AskUserQuestion：确认逆推结果/继续完善/放弃 |
| s04-code-contract | 从逆向结果生成契约 | contract-initializer (from-reverse) | standard | 300 | — | code | AskUserQuestion：确认/继续完善/放弃 |
| s05-code-testgen | 对抗性测试生成 | test-generator (模式A) | heavy | 600 | **yes** | code | 无门控 |
| s03-conflict-diff | 代码与设计差异对比 | diff-arbitrator (NEW, 模式B) | standard | 300 | — | conflict | 无门控 |
| s04-conflict-arbitrate | 差异仲裁 | diff-arbitrator (NEW, 模式C, continue) | standard | 300 | — | conflict | AskUserQuestion：确认裁决/继续仲裁/放弃 |
| s05-conflict-resolve | 按仲裁结果修正 | impl-executor (模式C) | heavy | 600 | **yes** | conflict | 无门控 |
| s06-conflict-validate | 修正后验证 | impl-validator | standard | 120 | — | conflict | 条件触发：重大风险时 AskUserQuestion |
| s07-conflict-test | 测试增量更新 | test-generator (模式C) | heavy | 600 | **yes** | conflict | 无门控 |
| s10-blindtest | 盲测执行与分支判定 | blindtest-runner | standard | 600 | — | 共用 | AskUserQuestion：all_pass/fix_impl/fix_test/recontract/放弃 |
| s11-fix-impl | 实现代码修复 | impl-executor (模式B, continue) | heavy | 600 | **yes** | 共用 | 无门控 |
| s12-fix-test | 测试缺陷修正 | test-generator (模式B, continue) | heavy | 600 | **yes** | 共用 | 无门控 |
| s13-report | 最终报告与验收 | report-generator | standard | 300 | — | 共用 | AskUserQuestion：接受并结束/追加修复 |
| s99-workflow-end | 工作流终止 | — (虚拟) | — | — | — | 共用 | — |

---

## 门控点分布（Skill 内部 AskUserQuestion）

> 所有门控由 Skill 通过 `AskUserQuestion` 实现，工作流层仅通过 `success` + `choice` 边路由。wfctl 将 AWAITING_CONFIRM 消息聚合后由主 Agent 呈现给用户。

| 门控 ID | Stage | 决策内容 | 类型 | 路径 |
|---------|-------|---------|------|------|
| AQ-004 | s02-path | 增量路径选择（自动推荐优先，边界时确认） | 终局确认（4路分支） | 共用 |
| AQ-001 | s03-full-init | 契约完整性 + 执行计划可接受性 | 终局确认 | full |
| AQ-002 | s05-full-validate | 重大风险验收（条件触发） | 条件触发 | full |
| AQ-001' | s04-incr-contract | 增量契约变更确认 | 终局确认 | incr |
| AQ-002' | s06-incr-validate | 增量变更重大风险验收（条件触发） | 条件触发 | incr |
...
| AQ-002'' | s06-conflict-validate | 修正后重大风险验收（条件触发） | 条件触发 | conflict |
| AQ-007 | s10-blindtest | 盲测分支信号确认（all_pass/fix_impl/fix_test/recontract） | 终局确认 | 共用 |
