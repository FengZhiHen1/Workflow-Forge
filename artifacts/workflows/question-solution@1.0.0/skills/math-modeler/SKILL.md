...
description: >
  ...
  当用户提到数学建模、公式推导、推导公式、推导数学公式、符号体系、符号表、假设评估、假设验证、可解性、误差分析、敏感性分析、参数敏感性、量纲分析、数学模型、建模文档、数学推导文档、数学建模文档时使用本 Skill。
...
- **版本控制**：版本记录表**仅出现在 `符号体系与假设` 文件中**，其余文件通过交叉引用指向该文件
...
- **所有从上游文档引用的参数，必须标注来源文档和具体位置**（如 `[模型选型_方案01_聚合ILP.md 第 2.3 节]`）
- **禁止凭记忆转写数值**：必须从源文档复制粘贴，不得手动重新输入
- **参数语义核对**：在引用参数前，必须确认该参数在上游文档中的数学定义与你的理解完全一致（不仅是数值一致，更要定义一致）
- **跨文件内部一致性**：同一符号/参数在多个文件中出现时，数值和定义必须完全一致
...
2. **确认 `[model]` 名称**：按以下优先级获取：
   - 首选：给定的命名约束（若提供）
   - 次选：从输入文件路径中解析模型简称（如 `模型选型_方案01_聚合ILP.md` → `聚合ILP`）
...
> - 符号体系、假设验证、数据映射**必须**单独成文（`符号体系与假设`），不得嵌入推导文档。
> - 可解性分析与数值方法**必须**单独成文（`公式推导_求解算法`），不得与问题建立合并。
> - 误差预算与敏感性分析**必须**单独成文（`误差与敏感性分析`），不得作为附录附在其他文件末尾。

| 序号 | 文件路径 | 内容 | 是否必须 |
|:---|:---|:---|:---|
| 1 | `VERSION_DOCS/符号体系与假设_[model].md` | 统一符号表 + 假设五维评估 + 数据映射表 | ✅ |
| 2 | `VERSION_DOCS/公式推导_问题建立_[model].md` | 从问题到目标函数/控制方程的完整推导 | ✅ |
| 3 | `VERSION_DOCS/公式推导_求解算法_[model].md` | 从控制方程到数值解的推导 + 可解性分析 | ✅（若需数值求解）|
| 4 | `VERSION_DOCS/误差与敏感性分析_[model].md` | 误差预算 + 敏感性矩阵 + 结构敏感性 | ✅ |
| 5 | `VERSION_DOCS/验证标准_[model].md` | 可检验性指标、阈值、Toy Model 验证结果 | 推荐 |

> 命名规范：`[model]` 必须从以下来源按优先级获取：
> 1. 给定的命名约束（首选）
> 2. 输入文件路径中解析出的模型名称（如 `模型选型_方案01_聚合ILP.md` → `聚合ILP`）
> 
...
| 文件 | 必须包含 | 严禁包含 |
|:---|:---|:---|
| `符号体系与假设` | 统一符号表、假设五维评估、数据映射表、版本记录 | 任何公式推导步骤 |
| `公式推导_问题建立` | 问题重述、逐步推导、完整数学模型、Toy Model 验证 | 假设评估（仅引用） |
| `公式推导_求解算法` | 可解性分析、数值方法选型、收敛性条件、计算资源预估 | 问题建立阶段的推导 |
| `误差与敏感性分析` | 误差预算表、敏感性矩阵、结构敏感性、鲁棒性结论 | 具体公式推导过程 |
| `验证标准` | 性能指标阈值、残差检验标准、计算性能阈值 | 任何推导内容 |

### 写作要求

- 每个文件必须独立自洽：读者只打开一个文件时，无需翻阅其他文件即可理解该部分的完整逻辑
- 允许交叉引用：如"目标函数定义参见 `公式推导_问题建立` 第 2.3 节"
- 版本记录表**仅出现在 `符号体系与假设` 文件中**，其他文件通过"本推导基于 `符号体系与假设` 中的符号定义"一句话引用即可
- `验证标准` 中的指标阈值，应作为后续验收阶段的量化依据
...
- **数据一致性不可跳过**：所有引用上游文档的数值必须与源文档逐字一致；同一参数在多个文件中的定义和数值必须一致；必须确认对上游参数的理解与上游原意相符
