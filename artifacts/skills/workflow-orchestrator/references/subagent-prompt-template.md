...
在 prompt 中追加以下指令（放在「上报」段之后，优先级高于默认的 AskUserQuestion 翻译规则）：

```
本 stage 设有确认点，你必须按以下两步流程执行：

1. 任务分析完成后，上报 AWAITING_CONFIRM 等待用户确认：
   python .claude/scripts/wfctl/main.py message write ... --status AWAITING_CONFIRM --questions "选项1" "选项2"
...
### continue after confirmation（中继确认后）

当 action 的 `confirmation_point = true` 且 SubAgent 从中继确认返回时（AWAITING_CONFIRM → PENDING → continue），在 continue prompt 末尾追加：

```
确认点提醒：上一轮你上报了 AWAITING_CONFIRM，用户已做出选择。本轮请根据用户选择完成最终产出，然后直接上报 DONE。不要再上报 AWAITING_CONFIRM。
```
| `<stage_task>` | WORKFLOW.yaml 中该 stage 的 `name` 字段（不暴露 stage_id） |
| `<user_choice>` | 用户在上游 `confirm` 中的 `--choice` 值 |
| `<feedback>` | 用户在上游 `confirm` 中的 `--feedback` 值（如有） |
