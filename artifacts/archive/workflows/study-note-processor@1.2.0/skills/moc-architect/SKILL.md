...
1. **通用格式规则**：读取 `references/obsidian-format-rules.md` — 所有领域通用的 Obsidian 格式规则（frontmatter 结构、双链、标题层级、列表、表格嵌套、高亮避让、文本格式化）
2. **领域配置**：读取 `references/domain-config/{domain}/` 下所有文件
   - `atom-classification.md` — 原子分类体系与 Callout 映射
   - `format-contract.md` — 领域特有格式契约（Callout 映射、类型特定格式规则如 LaTeX/代码规范）
   - `categories/{type}.md` — 各类别写作规范（按领域动态加载）
3. **输出目录规范**：读取 `references/output-directory-spec.md`
...
生成完成后，向编排器输出完成摘要（不上报 PENDING_CONFIRM，v1.1.1 已取消确认点）：
- 列出 H2 模块划分（含每模块原子点数）
- 汇总各类别原子点数量（按 `atom-classification.md` 分类总表统计/总计）
- 标出产物路径及书级 MOC 更新状态
- 若检测到潜在问题（如某模块原子点过少/过多、PDF 解析噪声），在摘要中标注 warning 供用户后续感知

> v1.1.1 变更：本 Skill 不再上报 `PENDING_CONFIRM`。MOC 文件写入 Vault 后直接结束，工作流自动进入 s03-atomic-produce。
...
- [ ] 书级 MOC 已创建或更新
- [ ] 遵循 `format-contract.md` 的类型特定格式规则（如 math 的 LaTeX 规范、programming 的代码规范等）
