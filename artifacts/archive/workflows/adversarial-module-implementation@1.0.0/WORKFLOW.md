# Module Lifecycle — 核心对抗验证流水线 v1.0.0
...
| 核心机制 | 信息隔离四铁律（ISO-001~004）保证测试独立性；对抗性测试（黑盒找漏洞）而非验证性测试（白盒证正确） |

### v1.0.0 相比 v1.0.0 的变更

- **聚焦核心**：移除 Group 2-4（模块审查/缺陷修复/验收测试，拆分为独立工作流）和 Group 5（Git Worktree Sync）
- **合并前置**：`orch-preflight` + `orch-contract` 合并为 `s01-init`（环境就绪 + 契约提取 + 用户确认，一次交互）
- **强化鲁棒性**：`s02-impl` / `s04-testgen` / `s06-fix` / `s07-testfix` 的 SubAgent 调用 max_attempts 提升为 2
- **新增用户可见性**：`s05-blindtest` 每轮输出进度摘要；`s08-report` 增加用户审查确认点
- **提前终止出口**：`s05-blindtest` 进度输出处允许用户跳过剩余修复，直接生成报告
...
flowchart TD
    ...
    %% ===== 核心流水线 =====
    S01["s01: 环境就绪与契约冻结<br/>✅ 确认点<br/>环境检查 + 契约提取 + 设计仲裁<br/>+ 执行计划预览"]
    ...
    S05["s05: 盲测执行与分支判定<br/>自动化中枢<br/>🔄 max_retry=2<br/>每轮输出进度摘要"]
    ...
    S08["s08: 最终报告与验收<br/>✅ 确认点<br/>14 项检查 + 隔离审计"]
...
| 说明 | 合并 v1 的 `orch-preflight` + `orch-contract`。<br/>1. 环境检查：Python 版本、脚本完整性、契约目录存在性<br/>2. 按 P0(落地规范)→P1(设计文档)→P2(项目结构文档) 提取接口契约<br/>3. 生成并冻结 contract-expectations.md<br/>4. 展示执行计划预览（模块信息、契约条目数、SubAgent 调度计划）<br/>5. 条件确认 AQ-004：多份文档对同一接口定义不一致时向用户确认 |
...
| 确认点 | 是（条件触发：仅存在"重大风险"条目时触发 AQ-003） |
| 重试 | max_attempts=1（纯验证分析） |
| 输入 | `function-signatures.json`、`pending-confirmations.md` |
| 输出 | 验证通过/失败判定 |
| 说明 | 1. 验证 function-signatures.json 格式与合规性<br/>2. 评估 pending-confirmations 风险等级<br/>3. 格式/合规不通过 → 退回 s02-impl（最多 3 次）<br/>4. 重大风险项 → 触发确认点 AQ-003，用户确认后继续或退回<br/>5. 超过 3 次退回 → 强制进入 s08-report（标注验证失败） |
...
| 说明 | 自动化判断-分支逻辑中枢。<br/>**流程**：运行测试 → classify_failures.py 分类 → 分支判定 → 进度输出。<br/>**分支逻辑**：<br/>- 全部通过 → s08-report<br/>- 实现漏洞 → s06-fix（生成 failure-summary）<br/>- 测试缺陷 / 契约矛盾 → s07-testfix（生成 test-defects）<br/>- 退化（上轮通过本轮失败）→ s06-fix（标记退化类型）<br/>- 收敛停滞（连续两轮失败集合相同）→ s08-report<br/>**每轮进度输出**（不阻塞）：第 N 轮结果、通过/失败数、判定方向。<br/>**用户提前终止**：进度输出时可选"跳过剩余修复，直接生成报告"。<br/>**隔离合规**：运行 generate_failure_summary.py + validate_failure_summary.py（阻断级）。 |
...
| 输入 | 全流程产物（contract-expectations、function-signatures、各轮 failure-summary 和 test-defects、green-seeking-reports） |
| 输出 | `adversarial-report.md` |
| 说明 | 1. 生成 adversarial-report.md（契约覆盖度、循环统计[轮次/失败数/修复数]、最终结论）<br/>2. 14 项验收检查清单<br/>3. 运行 check_isolation.py 审计 ISO 四铁律<br/>4. 用户审查确认（同意→终止；拒绝→须附拒绝原因→返回 s05-blindtest 追加修复） |
...
|------|----------|-----------|------|------|
| 1 | `adversarial-module-implementation-init` | s01-init | Phase 2 新建 | 合并原 preflight+contract，负责环境检查、契约提取、执行计划预览 |
| 2 | `adversarial-module-implementation-impl-executor` | s02-impl, s06-fix | Phase 2 改造 | 双模式 Skill：模式 A 实现落地，模式 B 修复迭代 |
| 3 | `adversarial-module-implementation-impl-validator` | s03-validate | Phase 2 改造 | 验证实现输出格式与合规性，条件确认重大风险 |
| 4 | `adversarial-module-implementation-test-generator` | s04-testgen, s07-testfix | Phase 2 改造 | 双模式 Skill：模式 A 对抗性测试生成，模式 B 测试缺陷修正 |
| 5 | `adversarial-module-implementation-blindtest` | s05-blindtest | Phase 2 改造 | 测试执行、失败分类、分支判定、进度输出、用户提前终止 |
| 6 | `adversarial-module-implementation-reporter` | s08-report | Phase 2 改造 | 生成对抗验证报告、14 项检查、隔离审计 |

### 已移除的 Skill

| Skill ID | 原因 |
|----------|------|
| `adversarial-module-implementation-preflight` | 合并入 `adversarial-module-implementation-init` |
| `adversarial-module-implementation-contract` | 合并入 `adversarial-module-implementation-init` |
| `adversarial-module-implementation-git-sync` | Group 5 整体移除（与模块生命周期业务无关） |
| `adversarial-module-implementation-reviewer` | 拆分为独立工作流（模块审查） |
| `adversarial-module-implementation-defect-fixer` | 拆分为独立工作流（缺陷修复） |
| `adversarial-module-implementation-test-writer` | 拆分为独立工作流（验收测试） |
...
| `subagent-prompts.md` | references | `references/` | orch-init, impl-executor, test-generator | orch-init |
| `failure-summary-format.md` | references | `references/` | blindtest, reporter | blindtest |
| `contract-extractor.md` | references | `references/` | orch-init | orch-init |
...
| s06-fix → s05-blindtest | 对抗循环超过 5 轮 | → s08-report | 强制生成报告，含完整循环统计 |
...
以下设计点在 Phase 1 末尾已与用户确认解决：

1. **[U1] s05-blindtest 双 failure 出边歧义消除**：✅ 编排器读取 Skill 返回的 `branch_target` 字段（`fix_impl` → s06-fix / `fix_test` → s07-testfix）。
