...
**产物目录**：本 Skill 的产物目录由 Task Package 的 `target_dir` 字段中指定。默认写入 `VERSION_SCRIPTS`（即 `v{N}/scripts/`）和 `VERSION_RESULTS`（即 `v{N}/results/`）。完整目录规范见 `references/directory-structure.md`。
