...
**产物目录**：本 Skill 的产物目录由 Task Package 的 `target_dir` 字段中指定。默认文档写入 `VERSION_DOCS`（即 `v{N}/docs/`），结果写入 `VERSION_RESULTS`（即 `v{N}/results/`）。完整目录规范见 `references/directory-structure.md`。
