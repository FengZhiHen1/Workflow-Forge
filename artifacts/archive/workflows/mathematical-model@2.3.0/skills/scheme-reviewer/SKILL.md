...
description: "数学建模工作流的方案对抗审查员。以小问为单位并行运行，每个小问独立启动本 Skill，以评委视角攻击 model-architect 为该小问产出的候选方案，主动寻找选型漏洞、方案遗漏和上游衔接缺陷。触发场景包括：(1) p2-scheme-design 完成后自动调用，(2) 用户要求审查候选建模方案的合理性，(3) 修复后的方案需要重新审查，(4) 模型选型报告存在明显疑点需要外部挑战者介入。"
---
