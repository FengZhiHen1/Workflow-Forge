...
description: >
  ...
  编写、子工作流嵌套、并发优化等场景。只要涉及多步骤任务编排、pipeline 设计、
  stage 编排、任务 orchestration、多阶段协作流程，即使客户未说"工作流"也应使用。不负责纯 Skill 文案优化（用 skill-creator）或性能
...
| 行为 | 后果 | 正确做法 |
|------|------|---------|
| 跳过质量校验就交付 | 产出物不合格 | L1/L2/边界扫描全部通过才能交付（用户说"不用检查"也必须执行） |
| 用户未确认就转正 | 违反"无审批不修改"原则 | 展示 diff → AskUserQuestion → 确认后落盘 |
| 在原版本目录上直接修改新版本 | 历史版本丢失 | 复制到新版本目录，原版本完整保留 |
| 忽略旧 Skill 捆绑资源 | 有损改造 | 逐项迁移 references/scripts/assets，标记 ✅ 的文件必须复制并适配 |
| 忽略子工作流 | 父工作流质量不可信 | 递归检查所有嵌套工作流，含 `workflow` 字段的 Stage 必须分析子工作流骨架 |

### 🟡 操作错误（修正后可继续）

| 场景 | 处理方式 |
|------|---------|
| 用户指定路径不存在 | 立即报告，扫描候选路径，不继续 |
| 校验发现问题（L1/L2/边界扫描/critical 自评） | 定位具体错误 → 修正 → 重新校验/扫描，直到通过 |
| 信息不足以做设计判断 | 标注 `⚠️ UNCERTAIN`，向用户澄清 |
| Stage 结构问题（调整幅度 ≤30%） | 局部更新 WORKFLOW.yaml → 同步更新 manifest → 继续 |
| Stage 结构问题（调整幅度 >30%） | 重新设计 |
| 用户否决某项设计建议 | 决策文档中标记 ❌，继续其他维度 |
| 用户中途改变主意 | 更新决策文档，重新评估受影响范围 |

### 🟢 产物规范（确保交付物完整可用）

- 每个 references/scripts/assets 文件必须在 SKILL.md 中被引用——禁止产出孤立文件
- 决策文档、中间草稿、review 报告留在 `$WD/`，不进入 `artifacts/workflows/`
